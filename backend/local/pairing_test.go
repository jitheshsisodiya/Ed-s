...
func TestPairingEnrolsADeviceWithoutItsOwnPassword(t *testing.T) {
	...
	postJSON(t, base+"/api/v1/auth/login", "", map[string]string{
		"email":    "someone@example.com",
		"password": "a-long-enough-password",
	}, &session)
	...
	postJSON(t, base+"/api/v1/auth/pair", access, map[string]string{}, &pairing)
	...
	postJSON(t, base+"/api/v1/auth/pair/claim", "", map[string]string{"token": token}, &claimed)
...
func TestPairingCodeIsSpentOnFirstUse(t *testing.T) {
	...
	postJSON(t, base+"/api/v1/auth/pair/claim", "", map[string]string{"token": token}, nil)

	// The second attempt is the one an onlooker would make.
	status := postJSONStatus(t, base+"/api/v1/auth/pair/claim", "", map[string]string{"token": token})
...
func TestPairingCannotBeStartedWithoutASession(t *testing.T) {
	...
	if got := postJSONStatus(t, base+"/api/v1/auth/pair", "", map[string]string{}); got != http.StatusUnauthorized {
...
func postJSON(t *testing.T, url, bearer string, body any, out any) {
	t.Helper()
	status, blob := doPostJSON(t, url, bearer, body)
...
func postJSONStatus(t *testing.T, url, bearer string, body any) int {
	t.Helper()
	status, _ := doPostJSON(t, url, bearer, body)
	return status
}

func doPostJSON(t *testing.T, url, bearer string, body any) (int, []byte) {
	t.Helper()
